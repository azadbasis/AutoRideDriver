...
import org.autoride.driver.listeners.ProgressListener;
...
public class WebConnector {
    ...
    public WebConnector(StringBuilder url, String protocol, HashMap<String, String> params, JSONObject postData) {
        super();
        this.url = url;
    ...
    public WebConnector(StringBuilder url, String protocol, HashMap<String, String> params, JSONObject postData, List<String> fileList) {
        super();
        this.url = url;
        this.protocol = protocol;
        this.params = params;
...
//            urlConnection.setRequestProperty("User-Agent", AppConstants.USER_AGENT_CCAVENUE);
            ...
            if (Config.getInstance().getAccessToken() != null && !Config.getInstance().getAccessToken().equalsIgnoreCase("")) {
                urlConnection.setRequestProperty("access_token", Config.getInstance().getAccessToken());
            }

            if (Config.getInstance().getRememberToken() != null && !Config.getInstance().getRememberToken().equalsIgnoreCase("")) {
                urlConnection.setRequestProperty("rememberToken", Config.getInstance().getRememberToken());
            }
...
//        builder.addFormDataPart(postName, postData.toString());
        ...
        builder.addFormDataPart("auth_token", Config.getInstance().getAccessToken());
        switch (postName) {
            case "profile_update":
...
//            rBuilder.addHeader("Authorization", /*"Bearer " +*/ Config.getInstance().getAuthToken());
            rBuilder.addHeader("Auth", /*"Bearer " +*/ Config.getInstance().getAccessToken());
...
//                urlConnection.setRequestProperty("Authorization", /*"Bearer " +*/ Config.getInstance().getAuthToken());
                urlConnection.setRequestProperty("Auth", /*"Bearer " +*/ Config.getInstance().getAccessToken());
            urlConnection.setConnectTimeout(60000);
            urlConnection.setReadTimeout(60000);
...
//            rBuilder.addHeader("Authorization", /*"Bearer " +*/ Config.getInstance().getAuthToken());
    ...
    public String connectToGET_service(boolean shouldPrintResponse) {
        ...
        urlBuilder.addQueryParameter("userId", Config.getInstance().getUserID());

        Request.Builder rBuilder = new Request.Builder().url(urlBuilder.build())
                .addHeader("Accept", "application/json")
//                .addHeader("Connection", "Keep-Alive")
                .addHeader("Content-type", "application/json");
        if (Config.getInstance().getAccessToken() != null && !Config.getInstance().getAccessToken().equalsIgnoreCase("")) {
            rBuilder.addHeader("access_token", Config.getInstance().getAccessToken());
        }
        if (Config.getInstance().getRememberToken() != null && !Config.getInstance().getRememberToken().equalsIgnoreCase("")) {
            rBuilder.addHeader("rememberToken", Config.getInstance().getRememberToken());
        }
        rBuilder.get();
